      ...
      _setDesktopPosition: function(position) {
        if(!window.appInfo.isRunInMobile && this.domNode &&
          ...
          html.place(this.domNode, jimuConfig.mapId);
		        ...
		        /* jab - start of added section to support placing widgets in defined location on screen
         * 
		 'widget_top'   sets the widget distance from the top (header) section of the map
		 'widget_bottom'   sets the widget distance from the bottom of the map
         'widget_left'  sets the widget distance from the left edge of the map.  right position becomes 'auto'
         'widget_right' sets the widget distance from the right edge of the map. left position becomes 'auto'
        
         example section of application config.json file
                {
                	"uri": "widgets/ttPWS/Widget",
			        "config": "configs/ttPWS/config_PublicWaterSupply.json",
			        "name": "ttPWS",
        ...
        var bottom_position = 'auto';
		var left_position = position.left + 'px';
		var right_position = 'auto';
		if (this.position.widget_top)
		{
			top_position = this.position.widget_top + 'px';
			bottom_position = 'auto';
		}
		if (this.position.widget_bottom)
		{
			top_position = 'auto';
			bottom_position = this.position.widget_bottom + 'px';
		}		
		if (this.position.widget_left)
		{
			left_position = this.position.widget_left + 'px'; 
			right_position = 'auto';
		}
		if (this.position.widget_right)
		{
			left_position = 'auto';
			right_position = this.position.widget_right + 'px';
		}
